...
tool(
  'vm_threads',
  {
    title: 'Live thread inspector',
    description:
      'Every thread the sequencer knows about: target sprite (clone or ' +
      'original), hat that started it (topBlock), stack depth, status ' +
      '(running/yield/promise-wait/done) and kill flag. Use it to prove ' +
      'clones spawned: N threads with isClone on a target name after that ' +
      'target\'s spawn broadcast. Run `vm_load` first.',
    inputSchema: {},
  },
  () => runtime.threads(),
);
...
tool(
  'vm_monitors',
  {
    title: 'Full monitor table',
    description:
      'All monitors (visible or not): label, opcode, value, mode, owning ' +
      'sprite. Lets a caller watch a variable without pixels. Run ' +
      '`vm_load` first.',
    inputSchema: {},
  },
  () => runtime.monitors(),
...
tool(
  'vm_step_frame',
  {
    ...
    description:
      'Advance the VM exactly one frame, then return before/after thread ' +
      'counts, the full after-snapshot, and the delta (new threads, events ' +
      'fired on that tick). For per-clone proofs: step one frame after a ' +
      'hit and diff per-clone variables. Run `vm_load` first.',
    inputSchema: {},
  },
  () => runtime.stepFrame(),
);
...
tool(
  'vm_seed',
  {
    title: 'Deterministic PRNG seed',
    description:
      'Replace global Math.random with a mulberry32 stream (scratch-vm has ' +
      'no seedable RNG; every random call site hits the global directly). ' +
      'Same seed gives identical `pick random`/lane sequences across runs. ' +
      'Pass null to restore. Run `vm_load` first.',
    inputSchema: {
      seed: z
        .number()
        .int()
        .nullable()
        .optional()
        .describe('Integer seed, or null to restore Math.random.'),
    },
  },
  ({ seed }) => runtime.setSeed(seed ?? null),
);
...
tool(
  'vm_watch',
  {
    ...
    description:
      'Poll-and-diff variable watcher: reads live target variables and ' +
      'reports old/new/changed per watch key. Omit name to watch all ' +
      'Stage scalars; pass target for one sprite (clones own their values, ' +
      'so per-clone watches work). Run `vm_load` first.',
    inputSchema: {
      name: z
        .string()
        .optional()
        .describe('Variable name to watch (omit for all Stage scalars).'),
      target: z
        .string()
        .optional()
        .describe('Sprite name (omit for the Stage).'),
    },
  },
  (opts) => runtime.watch(opts),
);

tool(
  'vm_pen_png',
  {
    title: 'Pen canvas PNG',
    description:
      'The headless pen raster (480x360 stage coords) as PNG base64 plus a ' +
      'non-transparent pixel count. Pen strokes only — sprites and ' +
      'backdrops never render. Run `vm_load` first.',
    inputSchema: {},
  },
  () => runtime.penPng(),
);
...
tool(
  'vm_mix_wav',
  {
    title: 'Offline sound mix',
    description:
      'Render every sound played since load into one 16-bit mono WAV ' +
      '(base64): each play mixed at its project-timer offset, volume/pitch ' +
      'effects approximate. Empty mix yields 100 ms of silence. Run ' +
      '`vm_load` first.',
    inputSchema: {},
  },
  () => runtime.mixWav(),
...
tool(
  'vm_stub_calls',
  {
    title: 'Stub call log',
    description:
      'Recorded pen/sound stub calls since load: pen ops (clear/point/' +
      'line/stamp) and sound ops (play/stop). Proves pen/sound blocks ' +
      'executed headless. Run `vm_load` first.',
    inputSchema: {},
  },
  () => runtime.stubCallsOf(),
);
...
tool(
  'vm_run_until',
  {
    title: 'Run until a condition fires',
    description:
      'Advance the VM on virtual time (no per-frame sleep, no stdio ' +
      'round-trip per chunk) until ANY predicate matches or the budget ' +
      'elapses: a live variable equals a value (varEquals), a broadcast ' +
      'name appears in the timeline (broadcastSeen), or no live threads ' +
      'remain (threadsIdle). Returns the same snapshot as `vm_run` plus ' +
      '`reached` (bool) and the echoed `predicate`. Replaces N× ' +
      '`vm_run`-poll loops. Run `vm_load` first.',
    inputSchema: {
      seconds: z
        .number()
        .positive()
        .optional()
        .describe('Virtual-time budget in seconds (default 10, max 60).'),
      chunkFrames: z
        .number()
        .int()
        .positive()
        .optional()
        .describe('Frames between predicate checks (default 15).'),
      until: z
        .object({
          varEquals: z
            .array(
              z.object({
                target: z
                  .string()
                  .optional()
                  .describe('Sprite name or "Stage" (default).'),
                name: z.string().describe('Variable name.'),
                value: z
                  .any()
                  .describe('Fires when the live value coerces equal.'),
              }),
            )
            .optional(),
          broadcastSeen: z
            .array(z.string())
            .optional()
            .describe('Broadcast names — fires when one is seen.'),
          threadsIdle: z
            .boolean()
            .optional()
            .describe('Fires when no live threads remain.'),
        })
        .optional()
        .describe('Fires when ANY entry matches (OR).'),
    },
  },
  (opts) => runtime.runUntil(opts),
);

tool(
  'vm_poke',
  {
    title: 'Set live VM state',
    description:
      'Write through to the RUNNING VM (fault injection mid-wave): set ' +
      'live variables/lists by sprite name or "Stage", teleport sprites ' +
      '(x/y/direction/size/visible/costume). Unlike `set_variable`, which ' +
      'edits project JSON on disk, this changes what the running scripts ' +
      'see. Unknown target/variable/list/costume is an error. Run ' +
      '`vm_load` first.',
    inputSchema: {
      variables: z
        .array(
      ...
      sprites: z
        .array(
          z.object({
            ...
            size: z.number().optional(),
            visible: z.boolean().optional(),
            costume: z.union([z.string(), z.number()]).optional(),
          }),
        )
        .optional(),
    },
  },
  (opts) => runtime.poke(opts),
);

tool(
  'vm_clones',
  {
    title: 'Live clone census',
    description:
      'Every live clone in one view: sprite name, position, costume, ' +
      'visibility and sprite-local variables (MyIndex, Kind, …). Threads ' +
      'show scripts, state shows originals — this shows the clone ' +
      'population itself. Run `vm_load` first.',
    inputSchema: {},
  },
  () => runtime.clones(),
);
...
tool(
  'find_blocks',
  {
    title: 'Find blocks across targets',
    description:
      'Query every target\'s `blocks` map in the open project: filter by ' +
      'opcode (exact or substring), by field value (e.g. VARIABLE name), ' +
      'by input name, or by hat/string content. Returns matching blocks ' +
      'with their target, opcode, parent/next links and input/field ' +
      'names — the read half of surgical editing, before `patch_target`. ' +
      'Needs an open project (`open_project` first), not the VM.',
    inputSchema: {
      opcode: z
        .string()
        .optional()
        .describe('Match opcodes containing this string (exact wins first).'),
      fieldValue: z
        .string()
        .optional()
        .describe('Match blocks with a field holding this value.'),
      inputName: z
        .string()
        .optional()
        .describe('Match blocks having this input name.'),
      hat: z
        .boolean()
        .optional()
        .describe('Only topLevel hat/hat-like blocks when true.'),
      limit: z
        .number()
        .int()
        .positive()
  ...
  ({ opcode, fieldValue, inputName, hat, limit }) => {
    const project = openProject();
    const max = Math.min(limit ?? 50, 200);
    const out = [];
    for (const t of project.json.targets) {
      const blocks = t.blocks ?? {};
      for (const [id, b] of Object.entries(blocks)) {
        if (!b || typeof b !== 'object' || Array.isArray(b)) continue;
        if (opcode && !(b.opcode || '').includes(opcode)) continue;
        if (inputName && !(b.inputs && inputName in b.inputs)) continue;
        if (fieldValue) {
          const fields = Object.values(b.fields ?? {});
          if (!fields.some(([v]) => String(v) === fieldValue)) continue;
        }
        if (hat && !b.topLevel) continue;
        out.push({
          target: t.name,
          id,
          opcode: b.opcode,
          ...
          next: b.next ?? null,
          inputs: Object.keys(b.inputs ?? {}),
          fields: Object.fromEntries(
            Object.entries(b.fields ?? {}).map(([k, v]) => [k, Array.isArray(v) ? v[0] : v]),
          ),
        });
        if (out.length >= max) return { matches: out, truncated: true };
      }
    }
...
tool(
  'validate_blocks',
  {
    title: 'Validate target blocks',
    description:
      'Run the sidecar\'s opcode/input catalog validator over one target ' +
      '(or every target): unknown standard opcodes (with did-you-mean), ' +
      'unexpected input/field names, out-of-range dropdown values, ' +
      'extension blocks whose extension is not enabled. Advisory — custom ' +
      'extension blocks are intentionally ignored. Same engine as the ' +
      '`warnings` on `patch_target`, callable on demand. Needs an open ' +
      'project, not the VM.',
    inputSchema: {
      target: z
        .string()
        .optional()
        .describe('Sprite name or "Stage" (omit for all targets).'),
    },
  },
  ({ target: targetName }) => {
    const project = openProject();
    const enabledExtensions = project.extensions;
    const scope = targetName
      ? [target(targetName).json]
      : project.json.targets;
    const warnings = [];
    for (const t of scope) {
      for (const w of validateBlocks(t.blocks, { enabledExtensions })) {
        warnings.push(`${t.name}: ${w}`);
      }
    }
    return { target: targetName ?? '(all)', warnings, clean: warnings.length === 0 };
  },
);
...
// The bridge is opt-in: only bind when SCRATCH_MCP_BRIDGE_PORT is set.
// A second sidecar spawn (or a stale one) on the same port used to throw
// EADDRINUSE as an unhandled error event and kill the process before MCP
// stdio ever came up — every sb3_* call then died with "node stdout closed".
if (process.env.SCRATCH_MCP_BRIDGE_PORT) {
  const bridgePort = Number(process.env.SCRATCH_MCP_BRIDGE_PORT);
  try {
